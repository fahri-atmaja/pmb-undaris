...
include "../koneksi.php";
                if(isset($_POST['nama'])){
                                ...
                                $loadpendaftar = mysql_query("SELECT * FROM pendaftar WHERE id_regist=$id_regist");
...
'$kdpendidikana','$kdpekerjaana','$penghasilanayah','$nmibu','$kdpendidikani','$kdpekerjaani','$nohp','$statusdaftar','$id_sks')");
 $ujian = mysql_query("insert into m_siswa values('','$namacalon','$email','PMB 2022')");
                ...
                if (empty($cek)){echo "<script>alert('SILAHKAN MEMBAYAR BIAYA PENDAFTARAN UNTUK MELANJUTKAN TAHAP SELANJUTNYA! ^.^');
                location.href='index.php?p=payment';</script>";
                }
                
                if ($dis['status']=='n'){
                    echo "<script>alert('SILAHKAN MEMBAYAR BIAYA PENDAFTARAN UNTUK MELANJUTKAN TAHAP SELANJUTNYA! ^.^');location.href='index.php?p=payment';</script>";
                } elseif ($dis['statusdaftar']=='x'){
                    echo "<script>alert('Mohon Maaf, Pembayaran anda dinyatakan invalid, silahkan menghubungi panitia PMB / datang langsung ke BAUK UNDARIS');history.go(-1);</script>";
                }
                ?>

                <?php
                $loadpendaftar = mysql_query("SELECT * FROM pendaftar WHERE email='$username'");
    ...
    <div class="row">
      ...
      <div class="modal-body">
        ...
        <table class="table " width="100%">
          ...
          <input type="hidden" name="konsentrasi_id" id="konsentrasi_id" value="<?= $dis['prodi2'] ?>">
            ...
            <tr>
                ...
                <td width="1%"> : </td>
                <td> <input id="email" name="email" placeholder="Email" class="form-control" type="text" value="<?= $dis['email'] ?>" readonly required ></td>
            </tr>
            <tr>
        ...
        *) Silahkan Upload Foto Anda <a target="_BLANK" href="http://pmb.undaris.ac.id/calonmahasiswa/index.php?p=passfoto">Klik Disini</a> Lalu refresh halaman ini. <br>
      ...
      <div class="modal-footer">
          <div class="container">
              <div class="row">
                  <div class="col-md-6">
                              <a target="_BLANK" href="http://pmb.undaris.ac.id/calonmahasiswa/index.php?p=passfoto"><button type="button" class="btn btn-danger"><span class='fa fa-print'></span> Upload Pass Foto</button></a>
    ...
    <?php
        ...
        $loadregist = mysql_query("SELECT * FROM regist WHERE email='$username'");
...
<div class="container">
 ...
 <tr>
          ...
          <tr>
            ...
            <input type="hidden" name="statusdaftar" value="tahap seleksi">
            <input type="hidden" name="id_sks" id="konsentrasi_id" value="<?= $diss['id_sks'] ?>">
        </table>
        *) Info Ujian Penerimaan Mahasiswa Baru akan langsung diinfokan setelah anda mendaftar <br> 
