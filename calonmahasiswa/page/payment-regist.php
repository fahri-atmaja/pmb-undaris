...
<?php
    ...
    $jenis = $fill['jenis_bayar'];
    // $cek        = mysql_num_rows($load);
    //     if($cek > 0){
    //         echo "<script>alert('TRANSAKSI SUDAH ADA !!'); location.href='$url/calonmahasiswa/index.php?p=statusbayar';</script>";
    //     }
    if($konsentrasi==25){
             $potongan = mysql_query("SELECT * FROM potongan WHERE angkatan_id='$angkatan' AND konsentrasi_id='25' AND jenis_bayar='$jenis' AND status='y'"); 
            }else{
            $potongan = mysql_query("SELECT * FROM potongan WHERE angkatan_id='$angkatan' AND konsentrasi_id='0' AND jenis_bayar='$jenis' AND status='y'");
            }
            $pot = mysql_fetch_array($potongan);
?>
<div class="container">
    <div class="row">
        <div class="col-md-6">
           ...
           <div class="table-responsive">
            <table id="dtHorizontalExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
                <tr>
                   <td>Jenis Bayar</td><td>:</td><td>Pembayaran <?= $fill['keterangan']; ?> - <?= $pot['nama']; ?></td>
                </tr>
