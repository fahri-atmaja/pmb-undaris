...
$date = date('Y-m-d');
                if(isset($_POST['submit'])){
                            ...
                            if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                                if($ukuran < 2570000){          
                                    $namabaru = "beasiswa-ppa-".$username."-".$nama;
                                    move_uploaded_file($file_tmp, 'berkasbeasiswa/'.$namabaru);
                                    $log = mysql_query("INSERT INTO daftar_beasiswa VALUES('','$username','$jenis','$namabaru','x')");
                                    if($log){
                                        // echo 'FILE BERHASIL DI UPLOAD';
                                        echo "<script>alert('FILE BERHASIL DIUPLOAD !!'); location.href='index.php?p=daftar-beasiswa';</script>";
                                    }else{
...
<body>
<h4 class="modal-title" id="myModalLabel">Upload Berkas Beasiswa PPA</h4>
    <div class="area-loading"></div>
      <div class="modal-body">
          <?php
            $loadtolak = mysql_query("SELECT * FROM daftar_beasiswa WHERE email='$username' AND status='n'");
            $cektolak = mysql_num_rows($loadtolak);
                    if ($cektolak > 0){
                        $view = mysql_fetch_array($loadtolak);
                        $target = "berkasbeasiswa/".$view['berkas']; 
                        unlink($target);
                        $filltolak = mysql_query("DELETE FROM daftar_beasiswa WHERE email='$username' AND status='n'");
                        echo "Berkas anda ditolak, silahkan upload ulang.";
                    }
        $loadsiswa = mysql_query("SELECT * FROM m_siswa WHERE nim='$username'");
        $v         = mysql_fetch_array($loadsiswa);
        $iduser    = $v['id'];
        $cekujian  = mysql_query("SELECT * FROM tr_ikut_ujian WHERE id_user='$iduser'");
        $cekdata   = mysql_num_rows($cekujian);
        $loadreg   = mysql_query("SELECT * FROM daftar_beasiswa WHERE email='$username'");
        $text      = mysql_fetch_array($loadreg);
        if($text['status']=='y'){
        ?>
        <?php
        if($cekdata > 0){
        echo "Lanjut registrasi";
        }else{
        ?>
        <label>Anda lolos ujian, silahkan menunggu jadwal untuk datang ke kampus. Info akan disampaikan ke nomor pendaftar.</label>
        <p>silahkan klik tombol dibawah : </p><br>
        <a href="?p=skip-beasiswa-hafidz&id=<?= $iduser ?>&email=<?= $username ?>"><button type="submit" name="submit" class="btn btn-success">LEWATI UJIAN</button></a>
        <?php
        }
        }else{
        ?>
          ...
          $load = mysql_query("SELECT * FROM daftar_beasiswa WHERE email='$username'");
        ...
        <table class="table " width="100%">
            <tr>
                ...
                <input type="hidden" name="jenis" value="ppa">
            </tr>           
        </table>
        <h5>PERHATIAN! UPLOAD JADIKAN 1 FILE DENGAN FORMAT .PDF DENGAN UKURAN FILE KURANG DARI 2MB<br>
        *) SCAN IJASAH, RAPORT KELAS X & XI BERISI PERINGKAT DAN RATA-RATA NILAI<br>
        <hr>
        ...
        ?>
        <h3><b>ANDA SUDAH UPLOAD BERKAS CEK STATUS ANDA AKAN DIUPDATE DISINI</b></h3>
        <hr>
        <?php
          }
        }
