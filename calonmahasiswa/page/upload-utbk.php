                        ...
                        $filltolak = mysql_query("DELETE FROM tolakberkas WHERE email='$username'");
                            ...
                            if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                                if($ukuran < 2570000){          
                                    ...
                                    move_uploaded_file($file_tmp, 'berkas/'.$namabaru);
                                    $query = mysql_query("UPDATE pendaftar set berkas='$namabaru' WHERE email='$username'");
...
<body>
<h4 class="modal-title" id="myModalLabel">Upload Berkas UTBK</h4>
    <div class="area-loading"></div>
      <div class="modal-body">
          <?php
          $tolak = mysql_query("SELECT * FROM tolakberkas WHERE email='$username'");
          $cektolak = mysql_num_rows($tolak);
          if($cektolak > 0){
              $tex = mysql_fetch_array($tolak);
              echo "Berkas anda ditolak :".$tex['alasan'].". Silahkan upload ulang";
          }
          ...
          $loadreg = mysql_query("SELECT * FROM pendaftar WHERE email='$username'");
        $text = mysql_fetch_array($loadreg);
        if($text['statusdaftar']=='1'){
        ?>
        <?php
        if($cekdata > 0){
        echo "Cek Status Seleksi";
        }else{
        ?>
        <label>Anda lolos ujian jalur UTBK, silahkan klik tombol dibawah</label>
        ...
        <h5>PERHATIAN! UPLOAD FILE UTBK FORMAT .PDF DENGAN UKURAN FILE KURANG DARI 2MB/<br>
        ...
        ?>
        <h3><b>ANDA SUDAH UPLOAD BERKAS CEK STATUS UTBK ANDA DISINI</b></h3>
        <?php
          }
        }
        ?>
