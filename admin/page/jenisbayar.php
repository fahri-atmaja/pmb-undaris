 <ol class='breadcrumb'>
                    <li><a href='./'>Home</a></li>
                    <li class='active'>Jenis Pembayaran PMB</li>
                </ol>
                <!-- end:breadcrumb -->   
  ...
  $("body").on("click", ".hapus", function(e) {
                ...
                $(".id_bayar").html($(DbNumberID).html());
                $('.dialog-hapus').modal('toggle');
                $("#konf").attr("href","hapusjenisbayar.php?id_bayar="+$(DbNumberID).html());
            });
    ...
    $("body").on("click", ".edit", function(e) {
                ...
                var id_bayar = $(kelas1).html();
                var tagihan = $(kelas2).html();
                var jumlah = $(kelas3).html();
                $("#id_bayar").val(id_bayar);
                $("#tagihan").val(tagihan);
                $("#jumlah").val(jumlah);
                $("#id_bayar").attr("readonly","true");
                $('.edit-dialog').modal('toggle');
            });
  ...
  <div class="modal-dialog">
    <div class="modal-content">
        ...
        <div class="modal-body">
            Apakah anda ingn menghapus data dengan kode Pembayaran =  <span class='id_bayar'></span>?
        </div>
...
<div class="modal fade bs-example-modal-lg edit-dialog" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" id="form" action="insertjenisbayar.php">
        <table class="table table-striped" width="100%">
            <!-- <tr>
                <th width="25%">Kode agama</th>
                <td width="1%"> : </td>
                <td> <input id="id_bayar" name="id_bayar" placeholder="Kode" class="form-control" type="hidden" ></td>
            </tr> --><tr>
                <th width="25%">Nama Tagihan</th>
                <td width="1%"> : </td>
                <td> <input id="tagihan" name="tagihan" placeholder="Nama" class="form-control" type="text" required ></td>
                <th width="25%">Jumlah</th>
                <td width="1%"> : </td>
                <td> <input id="jumlah" name="jumlah" placeholder="Jumlah bayar" class="form-control" type="text" required ></td>
            </tr>
        </table>
    ...
    <?php
        ...
        {
            if($_GET['code']==1)
                echo "<h3 style='color:green'>Data Jenis Bayar berhasil disimpan</h3>";
            elseif($_GET['code']==2)
                echo "<h3 style='color:red'>Terjadi kesalahan</h3>";
            elseif($_GET['code']==3)
                echo "<h3 style='color:blue'>Data Jenis Bayar berhasil dihapus</h3>";
        }
    ?>
<table class="table table-striped">
    ...
    <?php
        if(!isset($_GET['data']))
        $exec=mysql_query("select * from jenisbayar ");
        else{
            $data=trim(addslashes($_GET['data']));
            $exec=mysql_query("select * from jenisbayar where id_bayar like '%".$data."%' or tagihan like '%".$data."%'");
        }
