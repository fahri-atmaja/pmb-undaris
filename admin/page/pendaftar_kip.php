...
<table class="table table-striped">
    ...
    <?php
        ...
        if(!isset($_POST['data'])){
        $exec=mysql_query("select * from pendaftar_kip LIMIT $mulai, $halaman ");
        $sql = mysql_query("select * from pendaftar_kip");
        $total = mysql_num_rows($sql);
        $pages = ceil($total/$halaman); 
        }else{
            $data=trim(addslashes($_POST['data']));
            $exec=mysql_query("select * from pendaftar_kip where email like '%".$data."%' or nama like '%".$data."%'");
        }
    ...
    <tr>
        ...
        <td>
            ...
            if($r['berkas']){
            ?>
            <a target="_BLANK" href="https://drive.google.com/viewerng/viewer?embedded=true&url=https://pmb.undaris.ac.id/kip/berkaskip/<?= $r['berkas'] ?>"><button type="button" class="btn btn-danger" >View BERKAS</button></a>
            <?php
            if($r['statusdaftar']==0){
            ?>
            <a href="lolosberkas.php?id=<?= $r['email']; ?>"><button type="button" class="btn btn-success" >LOLOS BERKAS</button></a>
            <a href="?p=gagalberkas&id=<?= $r['email']; ?>"><button type="button" class="btn btn-warning" >GAGAL BERKAS</button></a>
            <?php
