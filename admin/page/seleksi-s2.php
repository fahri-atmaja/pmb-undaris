...
<!--<input type="text" class="form-control input-sm search" name="cari" placeholder="Search">-->
<!--<button type="submit" class="btn-primary">Cari Berdasarkan Nama Pendaftar</button>-->
<!--</form>-->
...
<table class="table table-striped">
    ...
    <?php
        if(isset($_GET['data'])){
        $data = trim($_GET['data']);
        $exec=mysql_query("select pendaftar.*, tbl_nilai.score from pendaftar
                            left join tbl_nilai on tbl_nilai.id_user=pendaftar.id_regist
                            where nama like '%".$data."%'");
        }
        else{
            
        $exec=mysql_query("select pendaftar.*, akademik_prodi.nama_prodi, tbl_nilai.score from pendaftar
                            left join tbl_nilai on tbl_nilai.id_user=pendaftar.id_regist
                            left join regist on regist.id_regist=pendaftar.id_regist
                            left join akademik_prodi on akademik_prodi.prodi_id=pendaftar.prodi_id
                            where regist.tahun=2021 and pendaftar.prodi_id=22");
        }
    ...
    <tr>
        ...
        <td>
            <span class="data-1-<?php echo $no; ?>"><?php echo $r['nama_prodi']; ?></span>
        </td>
        <td>
        ...
        <td>
            ...
            <a href="terima-s2.php?kdpendaftar=<?php echo $r[0]  ?>"><button type="button" class="btn btn-warning"><span class='fa fa-user'></span> Terima</button></a>
             <a href="tolak-s2.php?kdpendaftar=<?php echo $r[0]  ?>" ><button type="button" class="btn btn-danger"><span class='fa fa-user'></span> Tolak</button></a>
